...
void CWndResizer::OnSizeProc(UINT nType, int cx, int cy)
{
  ...
  root.OnResized();
  ResizeUI(&root);
	m_pHookedWnd->SendMessage(WM_RESIZER_SIZE);
...
void CWndResizer::OnLButtonUp(UINT nFlags, CPoint point)
{
  OnMouseMove(nFlags, point);
    m_pHookedWnd->SendMessage(WM_RESIZER_PORTION_CHANGE);
}
...
BOOL CWndResizer::Unhook()
{
 // ASSERT( m_pHookedWnd != NULL );
  ...
  m_pHookedWnd->SendMessage(WM_RESIZER_UNHOOK);
...
LRESULT CALLBACK CWndResizer::WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  ASSERT( pResizer != NULL);
	...
	switch (uMsg)
	{
      ...
      cx = LOWORD(lParam);    
      cy = HIWORD(lParam);    
      pResizer->OnSizeProc((UINT) wParam, cx, cy);
			break;
    case WM_SIZING:
