   ...
   ============================================================*/
{
	...
	AddFont(dc, "Calibri");
    AddFont(dc, "Consolas");
	AddFont(dc, "Courier New");
	AddFont(dc, "Andale Mono");
	AddFont(dc, "MS Sans Serif");
	AddFont(dc, "Tahoma");
	AddFont(dc, "Times New Roman");
	AddFont(dc, "±¼¸²");
}

void CFontComboBox::AddFont(CClientDC &dc, CString font)	
{	
	AddString(font);
	...
	// Add a "0" for the margin.
	CSize sz = dc.GetTextExtent(font + CString( "0" ) );

	SetMaxWidth( max(GetMaxWidth(), sz.cx ) );
