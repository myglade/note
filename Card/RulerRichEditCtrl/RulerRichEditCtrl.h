...
#define WM_LINK_CLICK		(WM_USER+1000)
#define WM_LINK_SET 		(WM_USER+1001)
...
// Attributes
	...
	void GetText(CString &s, int start, int end);
	
	CRichEditCtrl& GetRichEditCtrl( );

// Implementation
	void GetRTF(CString &rtf);
...
// Formatting
	...
	void SetFontName( const CString& font, int size = -1);
...
private:
	...
	BOOL	CreateToolbar();
	BOOL	CreateRuler();
	BOOL	CreateRTFControl(CString &font, int size, BOOL multiLine, BOOL autohscroll );
	void	CreateMargins();
